...
def generate_genome(length: int) -> Genome:
    return random.choices([1,0], k=length)
...
def fitness(genome: Genome, things: [Thing], weight_limit: int) -> int:
    if len(genome) != len(things):
        raise ValueError('genome and things must have same size.')
...
def single_point_crossover(a: Genome, b: Genome) -> Tuple[Genome, Genome]:
    if len(a) != len(b):
        raise ValueError('Genomes are not same size.')
...
def mutation(genome: Genome, num: int = 1, probability: float = .5) -> Genome:
    for _ in range(num):
